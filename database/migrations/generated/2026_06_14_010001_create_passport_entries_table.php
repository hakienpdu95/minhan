...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('passport_entries', function (Blueprint $table) {
            $table->id();
            $table->uuid('uuid')->unique()->comment('Public ID — dùng cho URL, API');
            ...
            $table->string('source_org_name', 200)->nullable()
        ...
        Schema::table('passport_entries', function (Blueprint $table) {
            $table->foreign('source_org_id', 'pe_source_org_fk')
                ->references('id')->on('organizations')->nullOnDelete();
            $table->foreign('org_verified_by_user_id', 'pe_verified_by_fk')

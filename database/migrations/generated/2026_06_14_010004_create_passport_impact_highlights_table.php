...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('passport_impact_highlights', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('source_impact_id')->nullable()
                ->comment('FK ai_impact_snapshots.id — nullable nếu record gốc bị xóa');
            $table->string('title', 300);
            $table->string('impact_category', 30)
        ...
        Schema::table('passport_impact_highlights', function (Blueprint $table) {
            $table->foreign('source_impact_id', 'pih_source_impact_fk')
                ->references('id')->on('ai_impact_snapshots')
                ->nullOnDelete();
        });
    }

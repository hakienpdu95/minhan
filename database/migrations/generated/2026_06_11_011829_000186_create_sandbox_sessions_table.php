    ...
    public function up(): void
    {
        Schema::create('sandbox_sessions', function (Blueprint $table) {
            ...
            $table->string('status', 20)->default('in_progress')
                ->comment('in_progress|submitted|evaluating|completed|abandoned');
            $table->timestamp('started_at');
            $table->timestamp('submitted_at')->nullable();
            $table->timestamp('completed_at')->nullable();
            $table->unsignedSmallInteger('duration_minutes')->nullable();

            // Điểm thành phần — Final Score = Quality×40% + Productivity×35% + AI Adoption×25%
            ...
            $table->foreign('sandbox_task_id')->references('id')->on('sandbox_tasks');
            $table->foreign('workforce_profile_id')->references('id')->on('workforce_profiles');

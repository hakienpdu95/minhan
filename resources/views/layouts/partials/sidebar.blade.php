<aside class="sidebar" id="sidebar">
    ...
    <nav class="nav-wrap">
        ...
        {{--
            ══════════════════════════════════════════════════════════════════
            BUSINESS CONSULTING OS — nhóm gộp mọi module tham gia trực tiếp
            luồng BCOS (xem spec/nghiencuu/bcos_master_flow.md Phần 8.1):
            Business Project (8 workspace), Lead (điểm vào + New Opportunity),
            Customer, Task (xương sống Delivery), Knowledge Center (KcCategory/
            KcItem — kho tri thức Knowledge Workspace), SOP (thư viện SOP).
            Mỗi mục vẫn giữ NGUYÊN điều kiện quyền hạn gốc của nó — chỉ gom
            chung 1 khu vực sidebar để luồng nghiệp vụ tư vấn liền mạch, không
            phải đi tìm rải rác ở nhiều section như trước.
            ══════════════════════════════════════════════════════════════════
        --}}
        @if(
            auth()->user()?->hasAnyPermission([
                'business_project.view','business_project.create',
                'leads.view_all','leads.view_assigned','leads.view_source',
                'customers.view_all','customers.view_assigned',
                'sop.view','sop.view_related','sop.create','sop.create_hr','sop.edit','sop.approve','sop.config',
            ])
            || auth()->user()?->can('viewAny', \Modules\Task\Models\Task::class)
            || auth()->user()?->can('viewAny', \Modules\KcCategory\Models\KcCategory::class)
            || auth()->user()?->can('viewAny', \Modules\KcItem\Models\KcItem::class)
            || auth()->user()?->can('viewAny', \Modules\KcItem\Models\KcTag::class)
        )
        <p class="section-title" style="margin-top:16px;">Business Consulting OS</p>
        <div class="nav-group">

            @if(auth()->user()?->hasAnyPermission(['business_project.view','business_project.create']))
            <details {{ request()->routeIs('backend.business-projects.*') ? 'open' : '' }}>
                <summary class="nav-summary {{ request()->routeIs('backend.business-projects.*') ? 'active' : '' }}">
                    <svg class="nav-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0H5m14 0h2M5 21H3m4-14h6m-6 4h6m-6 4h6"/></svg>
                    <span class="nav-label">Business Projects</span>
                    <svg class="nav-arrow" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="m9 18 6-6-6-6"/></svg>
                </summary>
                <div class="sub-menu">
                    <a href="{{ route('backend.business-projects.index') }}" class="sub-link {{ request()->routeIs('backend.business-projects.index') ? 'active' : '' }}">Danh sách Business Project</a>
                    @can('viewBcosDashboard', \Modules\BusinessProject\Models\BusinessProject::class)
                    <a href="{{ route('backend.business-projects.bcos-dashboard.show') }}" class="sub-link {{ request()->routeIs('backend.business-projects.bcos-dashboard.*') ? 'active' : '' }}">BCOS Dashboard</a>
                    @endcan
                    @can('viewAny', \Modules\BusinessProject\Models\DeliverableTemplate::class)
                    <a href="{{ route('backend.template-library.index') }}" class="sub-link {{ request()->routeIs('backend.template-library.*') ? 'active' : '' }}">Template Library</a>
                    @endcan
                </div>
            </details>
            @endif
        ...
        {{--
            ...
            thuộc nhóm "module khác" (Project/AI Copilot/RoleScope là công cụ
            nền tảng dùng nhiều nơi, không phải nghiệp vụ tư vấn cũng không
            phải khối thừa/HR/Marketplace).
            ══════════════════════════════════════════════════════════════════
        --}}
        ...
        {{--
            ...
            VÀ hiện chưa dùng thật trong vận hành: khối HR (Chi nhánh/Phòng ban/
            Chức danh/Nhân viên/Nghỉ phép/KPI/Đánh giá hiệu suất/Sơ đồ tổ chức),
            Recruitment/JobPosting, Marketplace, OCOP, Subscription/Billing,
            "Năng lực số" (Digital Twin/AI Sandbox/Certifications/Career...),
            và toàn bộ hệ sinh thái Deployment/BusinessSolution/Blueprint
            (Solution Catalog, Vertical Templates, Business Blueprint, Hub
            triển khai). Gom về cuối sidebar để không làm loãng luồng nghiệp
            vụ BCOS chính — KHÔNG xoá, chỉ hạ độ ưu tiên hiển thị.
            ══════════════════════════════════════════════════════════════════
        --}}
        ...
        <div class="nav-group">
            ...
            {{-- "Năng lực số" — Digital Twin/AI Sandbox/Certifications/Career pathway/AI Impact/
                 Career Journal/Assessment Marketplace + các trang admin liên quan --}}
            @auth
            ...
            {{-- Triển khai — Deployment hub + vertical dashboard (hệ sinh thái Deployment/
                 BusinessSolution/Blueprint, không thuộc luồng BCOS) --}}
            @auth

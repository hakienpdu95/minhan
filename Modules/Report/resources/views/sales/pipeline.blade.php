...
<div
    ...
    x-init="init()"
    class="p-6 space-y-6"
    data-api-url="{{ route('api.report.sales.pipeline') }}"
    data-source-options-url="{{ route('lead-source.list') }}"
>
...
<script>
const API_URL = '{{ route('api.report.sales.pipeline') }}';

document.addEventListener('alpine:init', () => {
    Alpine.data('reportSalesPipeline', () => ({
        loading: false,
        filters: {
            date_from: '',
            date_to:   '',
        ...
        _initTomSelect() {
            const el = this.$refs.sourceSelect;
            if (!el || !window.TomSelect) return;

            const sourceUrl = el.closest('[data-source-options-url]')?.dataset.sourceOptionsUrl
                           || el.closest('[data-api-url]')?.dataset.sourceOptionsUrl;

            this._tomSource = new TomSelect(el, {
                valueField:   'id',
                labelField:   'name',
                searchField:  ['name'],
                placeholder:  'Tất cả nguồn...',
                ...
                load(query, callback) {
                    const url = el.closest('[data-source-options-url]')?.dataset.sourceOptionsUrl
                              || document.querySelector('[data-source-options-url]')?.dataset.sourceOptionsUrl;
                ...
                onChange: (val) => { this.filters.source_id = val; },
            });
        },
        ...
        async load() {
            ...
            try {
                ...
                const url = (document.querySelector('[data-api-url]')?.dataset.apiUrl || API_URL)
                ...
                this.$nextTick(() => {
                    this._renderFunnelChart(data.funnel    ?? []);
        ...
        _renderFunnelChart(funnelData) {
            const el = document.getElementById('chart-pipeline-funnel');
            if (!el || !window.ECharts) return;

            if (!this._funnelChart) {
                const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
                this._funnelChart = window.ECharts.init(el, isDark ? 'dark' : null, { renderer: 'canvas' });
                new ResizeObserver(() => this._funnelChart?.resize()).observe(el);
            }

            if (!funnelData.length) {
                this._funnelChart.clear();
                return;
            }
            ...
            this._funnelChart.setOption({
                tooltip: {
                    trigger: 'item',
                    formatter: p => `<b>${p.name}</b><br/>Leads: ${(p.value ?? 0).toLocaleString('vi-VN')}<br/>Tỷ lệ: ${p.data.rate ?? '—'}%`,
                },
                ...
                yAxis: {
                    type: 'category',
                    data: funnelData.map(d => d.stage_name),
                    inverse: true,
                    axisLabel: {
                        fontSize: 12,
                        color: '#6b7280',
        ...
        _renderTrendChart(trendData) {
            const el = document.getElementById('chart-pipeline-trend');
            if (!el || !window.ECharts) return;

            if (!this._trendChart) {
                const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
                this._trendChart = window.ECharts.init(el, isDark ? 'dark' : null, { renderer: 'canvas' });
                new ResizeObserver(() => this._trendChart?.resize()).observe(el);
            }

            if (!trendData.length) {
                this._trendChart.clear();
                return;
            }
                ...
                series: [
                    {
                        name:        'Lead mới',
                        type:        'line',
                        data:        trendData.map(d => d.new_leads ?? 0),
                        smooth:      0.4,
                        symbol:      'circle',
                        symbolSize:  5,

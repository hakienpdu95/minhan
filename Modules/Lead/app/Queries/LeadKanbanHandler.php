...
class LeadKanbanHandler implements QueryHandlerInterface
{
    public function handle(QueryInterface $query): array
    {
        ...
        $cacheKey = $query->scopeUserId
            ? "lead_kanban:{$query->orgId}:user:{$query->scopeUserId}"
            : "lead_kanban:{$query->orgId}";
        ...
        try {
            return Cache::tags(["org:{$query->orgId}", 'kanban'])
                ->remember($cacheKey, config('lead.cache_ttl.kanban', 60), $loader);
        } catch (\BadMethodCallException) {
            return Cache::remember($cacheKey, config('lead.cache_ttl.kanban', 60), $loader);
        }
    }
}

...
<nav class="breadcrumb-nav">
    ...
    <span class="sep">›</span>
    <span class="current">Phòng ban</span>
</nav>
@endsection

@section('content')
<div x-data="deptListPage({{ Js::from([
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-bold text-base-content">Phòng ban</h1>
            <p class="text-sm text-base-content/50 mt-0.5">Quản lý cơ cấu phòng ban, bộ phận trong tổ chức</p>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Tổng phòng ban</div>
        ...
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Tạm dừng</div>
            <div class="stat-value text-2xl text-warning">{{ number_format($totalInactive) }}</div>
        </div>
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Đã sáp nhập</div>
            <div class="stat-value text-2xl text-base-content/40">{{ number_format($totalMerged) }}</div>
        </div>
    </div>
            ...
            <div class="flex flex-wrap gap-3 items-end">
                ...
                <div class="form-control flex-1 min-w-52">
                    <label class="label py-0.5">
                        ...
                        <span class="label-text-alt text-xs text-base-content/40">Tên, Mã, Mã chi phí</span>
                    </label>
                ...
                <div class="form-control w-48">
                    <label class="label py-0.5">
                        <span class="label-text text-xs font-medium">Chức năng</span>
                    </label>
                    <select id="filter-function" class="select select-sm select-bordered w-full"></select>
                </div>
...
<dialog id="deptDeleteModal" class="modal">
    <div class="modal-box max-w-sm">
        <h3 class="font-bold text-lg text-error">Xác nhận xóa</h3>
        <p class="py-3 text-sm text-base-content/70">
            Bạn có chắc muốn xóa phòng ban
            <strong id="deptDeleteName" class="text-base-content"></strong>?
        </p>
        <p class="text-xs text-error/70">Không thể xóa phòng ban còn phòng ban con hoặc đang hoạt động.</p>
        <div class="modal-action mt-4">
            <button id="deptDeleteConfirmBtn" class="btn btn-error btn-sm">Xóa</button>
            <button class="btn btn-ghost btn-sm" onclick="deptDeleteModal.close()">Hủy</button>
        </div>
    </div>

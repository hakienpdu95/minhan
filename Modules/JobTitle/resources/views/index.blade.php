...
<nav class="breadcrumb-nav">
    ...
    <span class="sep">›</span>
    <span class="current">Chức danh</span>
</nav>
@endsection

@section('content')
<div x-data="jobTitleListPage({{ Js::from([
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-bold text-base-content">Chức danh</h1>
            <p class="text-sm text-base-content/50 mt-0.5">Quản lý danh mục chức danh dùng chung toàn tổ chức</p>
        </div>
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('backend.job-titles.create') }}" class="btn btn-primary btn-sm gap-1.5">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Tổng chức danh</div>
            <div class="stat-value text-2xl">{{ number_format($totalAll) }}</div>
        </div>
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Đang dùng</div>
            <div class="stat-value text-2xl text-success">{{ number_format($totalActive) }}</div>
        </div>
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Vô hiệu</div>
            <div class="stat-value text-2xl text-warning">{{ number_format($totalInactive) }}</div>
        </div>
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Hệ thống</div>
            <div class="stat-value text-2xl text-info">{{ number_format($totalSystem) }}</div>
        </div>
    </div>
            ...
            <div class="flex flex-wrap gap-3 items-end">
                ...
                <div class="form-control flex-1 min-w-52">
                    <label class="label py-0.5">
                        ...
                        <span class="label-text-alt text-xs text-base-content/40">Tên, Mã chức danh</span>
                    </label>
                ...
                <div class="form-control w-48">
                    <label class="label py-0.5">
                        <span class="label-text text-xs font-medium">Nhóm chức danh</span>
                    </label>
...
<dialog id="jobTitleDeleteModal" class="modal">
    <div class="modal-box max-w-sm">
        <h3 class="font-bold text-lg text-error">Xác nhận xóa</h3>
        <p class="py-3 text-sm text-base-content/70">
            Bạn có chắc muốn xóa chức danh
            <strong id="jobTitleDeleteName" class="text-base-content"></strong>?
        </p>
        <p class="text-xs text-error/70">Không thể xóa chức danh đang được gán cho nhân viên hoặc bị khóa bởi hệ thống.</p>
        <div class="modal-action mt-4">
            <button id="jobTitleDeleteConfirmBtn" class="btn btn-error btn-sm">Xóa</button>
            <button class="btn btn-ghost btn-sm" onclick="jobTitleDeleteModal.close()">Hủy</button>
        </div>
    </div>

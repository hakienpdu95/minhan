...
<div x-data="projectListPage({{ Js::from([
    'apiUrl'      => route('backend.api.projects'),
    'statuses'    => $statuses,
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-bold text-base-content">Dự án</h1>
            <p class="text-sm text-base-content/50 mt-0.5">Quản lý dự án cross-functional của tổ chức</p>
        </div>
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('backend.projects.create') }}" class="btn btn-primary btn-sm gap-1.5">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Tổng dự án</div>
            <div class="stat-value text-2xl">{{ number_format($totalAll) }}</div>
        </div>
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Đang thực hiện</div>
            <div class="stat-value text-2xl text-success">{{ number_format($totalActive) }}</div>
        </div>
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Lên kế hoạch</div>
            <div class="stat-value text-2xl text-info">{{ number_format($totalPlanning) }}</div>
        </div>
        <div class="stat bg-base-100 border border-base-200 rounded-xl py-3 px-4 shadow-sm">
            <div class="stat-title text-xs">Hoàn thành / Hủy</div>
            <div class="stat-value text-2xl text-base-content/40">{{ number_format($totalDone) }}</div>
        </div>
    </div>
        ...
        <div class="card-body py-3 px-4 space-y-3">
            ...
            <div class="flex flex-wrap gap-3 items-end">
                ...
                <div class="form-control flex-1 min-w-52">
                    <label class="label py-0.5">
                        ...
                        <span class="label-text-alt text-xs text-base-content/40">Tên, Mã, Mô tả</span>
                    </label>
                ...
                <div class="form-control w-40">
                    <label class="label py-0.5">
                        <span class="label-text text-xs font-medium">Mức ưu tiên</span>
                    </label>
            ...
            <div class="flex flex-wrap gap-3 items-end">
                ...
                <div class="form-control w-64">
                    <label class="label py-0.5">
                        <span class="label-text text-xs font-medium">Ngày bắt đầu</span>
                    </label>
                    <div class="input input-sm input-bordered flex items-center gap-2 bg-base-100">
                        <svg class="w-3.5 h-3.5 text-base-content/40 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<dialog id="projectDeleteModal" class="modal">
    <div class="modal-box max-w-sm">
        <h3 class="font-bold text-lg text-error">Xác nhận xóa</h3>
        <p class="py-3 text-sm text-base-content/70">
            Bạn có chắc muốn xóa dự án
            <strong id="projectDeleteName" class="text-base-content"></strong>?
        </p>
        <p class="text-xs text-error/70">Thao tác này không thể hoàn tác.</p>
        <div class="modal-action mt-4">
            <button id="projectDeleteConfirmBtn" class="btn btn-error btn-sm">Xóa</button>
            <button class="btn btn-ghost btn-sm" onclick="projectDeleteModal.close()">Hủy</button>
        </div>
    </div>

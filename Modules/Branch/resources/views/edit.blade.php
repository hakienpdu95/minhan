...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-base-content/50 mt-0.5">
            {{ $branch->name }} <span class="font-mono opacity-60">({{ $branch->code }})</span>
        </p>
    </div>
...
<form method="POST" action="{{ route('backend.branches.update', $branch) }}" novalidate data-branch-form>
    @csrf
    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="form-control">
                            ...
                            <select id="ts-parent" name="parent_id"
                                    ...
                                    @if(!$orgLocked)
                                        data-parent-api="{{ route('api.branches.options') }}"
                                        data-selected-parent="{{ old('parent_id', $branch->parent_id) }}"
                                    @endif>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div class="form-control">
                            ...
                            <select id="ts-ward" name="ward_code"
                                    data-selected-ward="{{ old('ward_code', $branch->ward_code) }}"
            ...
            <div class="card bg-base-100 shadow-sm border border-base-200">
                <div class="card-body p-4">
                    ...
                    <div class="flex justify-between text-xs text-base-content/40 mb-4 px-0.5">
                        <span>Tạo {{ $branch->created_at?->format('d/m/Y') }}</span>
                        <span>Sửa {{ $branch->updated_at?->diffForHumans() }}</span>
                    </div>

                    <div class="flex gap-2">
                        <a href="{{ route('backend.branches.show', $branch) }}" class="btn btn-ghost btn-sm flex-1">Hủy</a>

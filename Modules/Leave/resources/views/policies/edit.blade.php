...
@section('content')
<div class="max-w-2xl">
    <h1 class="text-2xl font-bold mb-6">Sửa chính sách: {{ $policy->name }}</h1>
    ...
    <form method="POST" action="{{ route('backend.leave.policies.update', $policy) }}" class="space-y-4">
        @csrf @method('PUT')
        ...
        <div class="card bg-base-100 shadow-sm border border-base-200">
            <div class="card-body space-y-4">
                ...
                <div class="form-control">
                    <label class="label"><span class="label-text">Loại nghỉ</span></label>
                    <input type="text" value="{{ $policy->leave_type->label() }}"
                           class="input input-bordered bg-base-200" readonly>
                    <span class="text-xs text-base-content/50 mt-1">Không thể thay đổi loại nghỉ sau khi tạo</span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="form-control">
                        <label class="label"><span class="label-text">Ngày chuyển tiếp</span></label>
                    ...
                    <div class="form-control">
                        ...
                        <input type="number" name="max_consecutive_days"
                               value="{{ old('max_consecutive_days', $policy->max_consecutive_days) }}"
                               min="1" class="input input-bordered" placeholder="Không giới hạn">
                    </div>

                    <div class="form-control">
                        <label class="label"><span class="label-text">Hiệu lực từ ngày</span></label>
                    ...
                    <div class="form-control">
                        <label class="label cursor-pointer justify-start gap-3">
                            ...
                            <input type="checkbox" name="requires_approval" value="1" class="checkbox"
                                   @checked(old('requires_approval', $policy->requires_approval))>
            ...
            <div class="card-body space-y-3">
                ...
                <div class="form-control">
                    ...
                    <select name="job_title_id" class="select select-bordered w-full">
                        ...
                        <option value="{{ $jt->id }}"
                                @selected(old('job_title_id', $policy->job_title_id) == $jt->id)>
                            {{ $jt->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="form-control">
                    ...
                    <select name="department_id" class="select select-bordered w-full">
                        ...
                        <option value="{{ $dept->id }}"
                                @selected(old('department_id', $policy->department_id) == $dept->id)>
                            {{ $dept->name }}
                        </option>
                        @endforeach
                    </select>
